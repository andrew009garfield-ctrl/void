#!/usr/bin/env python3
"""Generic local shim that lets Void's Self-Hosted transcription talk
to an ASR backend that does NOT speak the OpenAI /audio/transcriptions
protocol.

The contract Void speaks (verified against its source):
  ...
  - Do not require Authorization from Void. Hold vendor keys in env vars.
...
def transcribe(audio_path: str, model: str, language: str | None, prompt: str | None) -> str:
    ...
    `model`/`language`/`prompt` come straight from Void's UI and may be
...
class ShimHandler(BaseHTTPRequestHandler):
    """Handles the single POST endpoint Void calls."""
...
def main() -> None:
    ...
    print("Point Void at it: Settings -> Transcription -> Self-Hosted")
