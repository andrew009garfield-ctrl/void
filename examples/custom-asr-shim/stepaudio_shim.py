#!/usr/bin/env python3
"""Local shim that bridges Void's Self-Hosted transcription to
StepFun's StepAudio 2.5 ASR API.

Void POSTs OpenAI-style multipart/form-data and expects JSON {"text": ...}.
...
Then in Void: Settings -> Transcription -> Self-Hosted,
...
def transcribe(audio_path: str, model: str, language: str | None, prompt: str | None) -> str:
    ...
    `model`/`language` from Void are available but StepAudio is pinned to
