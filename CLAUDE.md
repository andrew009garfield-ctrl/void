# Void Technical Reference for AI Assistants

This document provides comprehensive technical details about the Void project architecture for AI assistants working on the codebase.
...
Void is an Electron-based desktop dictation application that uses whisper.cpp for speech-to-text transcription. It supports both local (privacy-focused) and cloud (OpenAI API) processing modes.
...
- **hyprlandShortcut.js**: Hyprland Wayland global shortcut integration
  - Uses D-Bus service to receive hotkey toggle commands (same `com.void.App` service)
...
- **linuxAutostart.js**: Launch-at-login on Linux via an XDG autostart entry
  - `app.setLoginItemSettings()` is a no-op on Linux, so the entry is written directly to `$XDG_CONFIG_HOME/autostart/void.desktop`, matching the executable name electron-builder packages under
...
- **cliBridge.js**: Loopback HTTP server on ports 8200–8219, bearer-token auth (token at `~/.void/cli-bridge.json`), 127.0.0.1-only. Used by the unified CLI to talk to a running desktop app. `POST /v1/transcribe` takes a file **path** (never audio) and runs the user's downloaded local model through `IPCHandlers.transcribeLocalFile`, approving the path with `approveAudioPath` first; `GET /v1/transcribe/models` lists local models with download state and the app's default (`localTranscriptionModels.js`, read from the `.env` pre-warm values).
...
- **ReasoningService.ts**: AI processing for agent-addressed commands
  ...
  - Provider implementations live in a registry at `src/services/ai/inferenceProviders/index.ts` covering 8 providers (`anthropic`, `enterprise`, `gemini`, `groq`, `lan`, `local`, `openai`, `void`), each implementing the `InferenceProvider` interface from `types.ts`
...
- **whisper.js**: Native binary wrapper for local transcription
  ...
  - Models stored in `~/.cache/void/whisper-models/`
...
- **parakeet.js**: Model management for NVIDIA Parakeet ASR models
  ...
  - Models stored in `~/.cache/void/parakeet-models/`
...
- Qdrant data: `~/.cache/void/qdrant-data/` (`qdrant-data-dev/` in development)
- Qdrant binary: `resources/bin/qdrant-{platform}-{arch}` (bundled — downloaded during `prebuild` / `predev:main`)
- Embedding model: `~/.cache/void/embedding-models/all-MiniLM-L6-v2/` (auto-downloaded on first launch)
...
- **download-brand-fonts.js**: Fetches the licensed Yowza font files from the private `Void/brand-assets` release into the git-ignored `src/assets/fonts/yowza/`. Runs in `predev:main` and every `prebuild*` chain; skips (Noto Sans fallback) without repo access, fails only when `BRAND_FONTS_REQUIRED=1` (release CI)
...
Enable with `--log-level=debug` or `VOID_LOG_LEVEL=debug` (can be set in `.env`):
...
On GNOME Wayland, Electron's `globalShortcut` API doesn't work due to Wayland's security model. Void uses native GNOME shortcuts:
...
3. `gnomeShortcut.js` creates D-Bus service at `com.void.App`
...
- D-Bus service: `com.void.App`
- D-Bus path: `/com/void/App`
- gsettings path: `/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/void/`
...
On Hyprland (wlroots Wayland compositor), Electron's `globalShortcut` API and the `GlobalShortcutsPortal` feature don't work reliably. Void uses native Hyprland keybindings:
...
3. `hyprlandShortcut.js` creates D-Bus service at `com.void.App` (same as GNOME)
...
- Windows: `windows-mic-listener.exe` — WASAPI `IAudioSessionManager2` session monitoring; self-mic exclusion happens in JavaScript using current Void and capture-helper PIDs
...
- Routing: the press-time screenshot is re-decided against the model that will actually answer. On BYOK, it attaches only when the chat model's provider client is image-wired (`supportsImages` on the `InferenceProvider`) and the model has `supportsVision` in `modelRegistryData.json`; on Void Cloud, attachment is pending API vision routing for the chat path. Dropping the image never fails the command
...
7. **Local Semantic Search Not Working**:
   ...
   - Embedding model should be in `~/.cache/void/embedding-models/all-MiniLM-L6-v2/model.onnx` (auto-downloaded on first app launch)
...
- **Launch at login**: XDG autostart entry at `~/.config/autostart/void.desktop` (see `linuxAutostart.js`), since Electron's `setLoginItemSettings()` does nothing on Linux
