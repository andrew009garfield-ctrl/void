...
const policy = {
  version: 1,
  transcription: {
    allowedModes: ["enterprise", "void", "local"],
    allowedByokProviders: [],
    allowedEnterpriseProviders: ["azure"],
  },
  llm: { allowedModes: ["void"], allowedByokProviders: [], allowedEnterpriseProviders: [] },
...
test("managed transcription outranks Void Cloud and local Whisper on the upload lane", async (t) => {
  const { window } = installBrowserGlobals(t);
  const vite = await createRendererServer(t, {
    cachePrefix: "void-file-managed-",
  ...
  const errorResult = await transcribeFile("/tmp/a.wav", cfg({ isVoidCloud: true }), true);
