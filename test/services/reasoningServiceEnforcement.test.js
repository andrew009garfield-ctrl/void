...
test("ReasoningService entry points enforce the org policy", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-reasoning-enforcement-test-",
  });
  ...
  await t.test("dispatch-mode mapping: void is not smuggled through providers", async () => {
    setPolicy({ llmModes: ["providers"], llmByokProviders: ["openai"] });
    await assert.rejects(reasoningService.processText("hi", "", null, { provider: "void" }), {
      message: REASONING_RESTRICTED,
    });
  });
  ...
  await t.test("cloud agent streaming enforces the void mode", async () => {
    setPolicy({ llmModes: ["providers"], llmByokProviders: ["openai"] });
