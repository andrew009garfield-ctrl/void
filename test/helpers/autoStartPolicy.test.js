...
test("Windows and Linux detect a login launch from the flag on argv", () => {
  for (const platform of ["win32", "linux"]) {
    assert.equal(
      wasLaunchedHidden({ platform, argv: ["Void.exe", HIDDEN_LAUNCH_FLAG] }),
      true,
      platform
    );
    assert.equal(wasLaunchedHidden({ platform, argv: ["Void.exe"] }), false, platform);
  }
});
...
test("a relaunch drops the hidden-launch flag and keeps every other arg", () => {
  assert.deepEqual(
    getRelaunchOptions({
      argv: ["Void.exe", HIDDEN_LAUNCH_FLAG, "--log-level=debug"],
      protocol: "void",
    }),
...
test("a relaunch drops the deep link that cold-started the app", () => {
  assert.deepEqual(
    getRelaunchOptions({
      argv: [
        "Void.exe",
        "void://auth/callback?bearer_token=stale",
...
test("an AppImage relaunches from the AppImage file, not its FUSE mount", () => {
  assert.deepEqual(
    getRelaunchOptions({
      argv: ["/tmp/.mount_OpenWh/void", "--no-sandbox"],
      protocol: "void",
      appImagePath: "/home/user/Void.AppImage",
    }),
    { launcherPath: "/home/user/Void.AppImage", args: ["--no-sandbox"] }
  );
});
...
test("the Windows portable build relaunches from the portable exe, not its unpack dir", () => {
  assert.deepEqual(
    getRelaunchOptions({
      argv: ["C:\\Users\\me\\AppData\\Local\\Temp\\2abc\\Void.exe", HIDDEN_LAUNCH_FLAG],
      protocol: "void",
      portableExecutablePath: "D:\\Tools\\Void-1.10.0.exe",
    }),
    { launcherPath: "D:\\Tools\\Void-1.10.0.exe", args: [] }
  );
});
...
test("the AppImage waiter outlives this process, then execs the file with its args", () => {
  assert.deepEqual(
    getRelaunchWaiter({
      platform: "linux",
      launcherPath: "/home/user/Void.AppImage",
      args: ["--no-sandbox"],
      pid: 4242,
      ppid: 4200,
    }),
...
test("the portable waiter waits for the stub and quotes the path for PowerShell", () => {
  assert.deepEqual(
    getRelaunchWaiter({
      platform: "win32",
      launcherPath: "D:\\Tom's Tools\\Void.exe",
      args: ["--log-level=debug"],
      pid: 4242,
      ppid: 4200,
      systemRoot: "D:\\Win",
    }),
    {
      ...
      args: [
        ...
        "-NonInteractive",
        "-Command",
        "Wait-Process -Id 4200; Start-Process -FilePath 'D:\\Tom''s Tools\\Void.exe' -ArgumentList '--log-level=debug'",
      ],
    }
  );
});
  ...
  assert.deepEqual(
    ...
    }),
    {
      ...
      args: [
        ...
        "-NonInteractive",
        "-Command",
        "Wait-Process -Id 4200; Start-Process -FilePath 'D:\\Tools\\Void.exe'",
      ],
    }
  );
});
