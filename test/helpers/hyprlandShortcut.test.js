...
function withTempHyprConfig(fn) {
  return async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "void-hyprland-test-"));
...
const DBUS_COMMAND =
  "dbus-send --session --type=method_call --dest=com.void.App /com/void/App com.void.App.Toggle";
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.ok(
      fs
        ...
        .includes(`source = ${path.join(configDir, "void-binds.conf")}\n`)
    );
    assert.match(
      fs.readFileSync(path.join(configDir, "void-binds.conf"), "utf8"),
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.match(fs.readFileSync(luaPath, "utf8"), /pcall\(require, .+void-binds\.lua/);
    assert.equal(
      (fs.readFileSync(luaPath, "utf8").match(/void-binds\.lua/g) || []).length,
      1
    );
    const binds = fs.readFileSync(path.join(configDir, "void-binds.lua"), "utf8");
    assert.match(binds, /^-- Void keybinds/m);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(configDir, "void-binds.lua")), false);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(path.dirname(configPath), "void-binds.lua")), true);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    assert.equal(fs.existsSync(path.join(configDir, "void-binds.lua")), true);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    const luaPath = path.join(configDir, "hyprland.lua");
    const bindsPath = path.join(configDir, "void-binds.lua");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      luaPath,
      `-- lua config\ndofile("${bindsPath}")\nreturn {}\n-- trailing comment\n`
    ...
    assert.equal((content.match(/void-binds\.lua/g) || []).length, 1);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    const configPath = path.join(configDir, "hyprland.conf");
    const bindsPath = path.join(configDir, "void-binds.conf");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(configPath, "# legacy config\n");
    fs.writeFileSync(
      bindsPath,
      "# Void keybinds (managed automatically)\n" +
    ...
    assert.equal((content.match(/Void keybinds/g) || []).length, 1);
  })
);
...
test(
  ...
  withTempHyprConfig(async (configDir) => {
    ...
    const legacyBindsPath = path.join(configDir, "void-binds.conf");
    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(path.join(configDir, "hyprland.lua"), "-- lua config\n");
    fs.writeFileSync(confPath, "# keep this comment\nsource = ./void-binds.conf\n");
    fs.writeFileSync(
      legacyBindsPath,
      "# Void keybinds (managed automatically)\n" +
