...
function withRuntime({ platform, argv = [], env = {}, electron }, loadModule) {
  ...
  const previousEnvironment = {
    ...
    VOID_LOG_LEVEL: process.env.VOID_LOG_LEVEL,
  };
  ...
  delete process.env.VOID_LOG_LEVEL;
...
test("log-level settings never opt a packaged Windows build into console logging", async (t) => {
  const cases = [
    ...
    {
      name: "VOID_LOG_LEVEL=debug",
      env: { VOID_LOG_LEVEL: "debug" },
    },
    {
      name: "VOID_LOG_LEVEL=info",
      env: { VOID_LOG_LEVEL: "info" },
    },
...
test(
  ...
  async () => {
    const userDataPath = fs.mkdtempSync(path.join(os.tmpdir(), "void-debug-logger-"));
    const logger = loadLogger({
      platform: "win32",
      isPackaged: true,
      env: { VOID_LOG_LEVEL: "debug" },
