...
test("stripAgentAddress removes the leading cue and name, keeping the command", async () => {
  ...
  assert.equal(
    stripAgentAddress("Hey Void, make this formal", "Void"),
    "make this formal"
  );
  assert.equal(stripAgentAddress("Max take a note", "Max"), "take a note");
  assert.equal(
    stripAgentAddress("That's everything. Void, format this", "Void"),
    "That's everything. format this"
  );
  assert.equal(stripAgentAddress("make this formal", "Void"), "make this formal");
  assert.equal(
    stripAgentAddress("Void", "Void"),
    "Void",
...
test("matches the name when it starts the dictation", async () => {
  ...
  assert.equal(detectAgentName("Void, summarize this note", "Void"), true);
...
test("matches the name after a greeting cue", async () => {
  ...
  assert.equal(detectAgentName("hey Void make this formal", "Void"), true);
...
test("matches the name opening a new sentence", async () => {
  ...
  assert.equal(
    detectAgentName("That's everything. Void, format this as bullets", "Void"),
    true
  );
});
...
test("ignores mentions that are dictated content, not commands", async () => {
  ...
  assert.equal(detectAgentName("I showed Void to a friend yesterday", "Void"), false);
  assert.equal(detectAgentName("we shipped the Void update today", "Void"), false);
...
test("handles STT splitting or misspelling the name, with the same gating", async () => {
  ...
  // when addressed...
  assert.equal(detectAgentName("hey open whisper translate this", "Void"), true);
  assert.equal(detectAgentName("Open Whisper, take a note", "Void"), true);
  // ...but not as a mid-sentence mention.
  assert.equal(
    detectAgentName("people keep calling open whisper a dictation app", "Void"),
    false
  );
});
...
// start with the agent name ("void review"), the wake-word scan used to
...
test("a name inside a snippet trigger is not an address", async () => {
  const { detectAgentName } = await load();
  const snippets = [{ trigger: "void review", replacement: "Review the PR" }];

  assert.equal(
    detectAgentName("void review this PR", "Void", undefined, snippets),
    false
  );
  assert.equal(
    detectAgentName("That's done. void review this PR", "Void", undefined, snippets),
    false
  );
});
...
test("a real address still counts when the dictation also uses a snippet trigger", async () => {
  ...
  assert.equal(
    detectAgentName(
      "Hey Void, run void review on PR 5",
      "Void",
      undefined,
      snippets
    ),
...
test("stripAgentAddress removes the real address, not the snippet trigger", async () => {
  ...
  assert.equal(
    stripAgentAddress(
      "void review. Void summarize it",
      "Void",
      undefined,
      snippets
    ),
    "void review. summarize it"
  );
  assert.equal(
    stripAgentAddress("void review this PR", "Void", undefined, snippets),
    "void review this PR",
...
test("a trigger clipping part of a split name does not suppress the address", async () => {
  ...
  assert.equal(
    detectAgentName("open whispr summarize this", "Void", undefined, snippets),
    true
  );
  assert.equal(
    stripAgentAddress("open whispr summarize this", "Void", undefined, snippets),
    "summarize this"
  );
});
...
test("an explicit cue outranks a trigger the address happens to span", async () => {
  ...
  assert.equal(
    detectAgentName("Hey Void review this PR", "Void", undefined, snippets),
    true
  );
  assert.equal(
    stripAgentAddress("Hey Void review this PR", "Void", undefined, snippets),
    "review this PR"
  );
...
test("a name inside a snippet trigger is not an address under CJK normalization", async () => {
  ...
  for (const language of ["ja", "zh", "zh-CN", "zh-TW"]) {
    assert.equal(
      detectAgentName("void review this PR", "Void", language, snippets),
      false,
      language
    );
  }
