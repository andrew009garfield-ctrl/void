...
test("available managed mode resolves the Void provider", async () => {
...
test("translation provider: available managed mode routes to void", async () => {
...
test("translation provider: incomplete managed and provider modes fail closed", async () => {
  ...
  for (const translationMode of ["void", "providers", "enterprise"]) {
    assert.equal(
      resolveTranslationProviderId({
        isCloudTranslation: false,
        translationMode,
