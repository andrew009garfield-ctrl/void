...
function request(overrides = {}) {
  return {
    ...
    authHeaders: { Authorization: "Bearer void-session" },
...
test("deduplicates config and Azure token refreshes while keeping cloud tokens out of config", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-enterprise-"));
  t.after(() => fs.rmSync(tempDir, { recursive: true, force: true }));
  const calls = { config: 0, assertion: 0, azure: 0 };
  const proxyFetch = async (url) => {
    if (url.includes("/enterprise-providers/azure/assertion")) {
      calls.assertion += 1;
      return jsonResponse({ data: { assertion: "signed-void-assertion" } });
    }
  ...
  const tokenState = { token: "void-session", generation: 4 };
  ...
  assert.equal(JSON.stringify(left).includes("signed-void-assertion"), false);
...
test("Bedrock exchanges the workspace assertion for short-lived web identity credentials", async (t) => {
  ...
  config.providers = [
    {
      ...
      config: {
        roleArn: "arn:aws:iam::123456789012:role/Void",
        region: "us-west-2",
  ...
  const manager = createEnterpriseIdentityManager({
    ...
    tokenStore: {
      getState: () => ({ token: "void-session", generation: 4 }),
    },
  ...
  assert.deepEqual(providerInit, {
    roleArn: "arn:aws:iam::123456789012:role/Void",
    roleSessionName: `void-${workspaceId.slice(0, 8)}`,
...
test("assertion requests carry the inference scope being exercised", async (t) => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-enterprise-scope-"));
  t.after(() => fs.rmSync(tempDir, { recursive: true, force: true }));
  const bodies = [];
  const config = managedConfig();
  ...
  const manager = createEnterpriseIdentityManager({
    ...
    proxyFetch,
    tokenStore: { getState: () => ({ generation: 4, token: "void-session" }) },
  });
