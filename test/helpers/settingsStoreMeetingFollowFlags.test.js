...
// "void" with no fallback, so note recordings and note formatting went to
// Void Cloud for a Local-everywhere user. The store now copies after the
...
test("Note Recording modes survive the follow-flag migration", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-meeting-follow-flags-test-",
  });
  ...
  await t.test("legacy reasoning keys derive the same modes as before", async () => {
    const cases = [
      ...
      [{ cloudReasoningMode: "void", reasoningProvider: "llama" }, "void"],
      [{ reasoningProvider: "llama" }, "void"],
    ];
  ...
  // a clear error rather than silently using Void Cloud.
  await t.test(
  ...
  await t.test("a ≤1.6.7 Void Cloud profile stays on Void Cloud", async () => {
    const { state } = await load({
      useLocalWhisper: "false",
      cloudTranscriptionMode: "void",
      cloudReasoningMode: "void",
      isSignedIn: "true",
    });
    assert.equal(state.meetingTranscriptionMode, "void");
    assert.equal(state.noteFormattingMode, "void");
  });
  ...
  await t.test(
    ...
    async () => {
      const { state } = await load({});
      assert.equal(storage.getItem("meetingTranscriptionMode"), "void");
      assert.equal(storage.getItem("noteFormattingMode"), "void");
  ...
  await t.test("a latched cloud profile keeps the cloud it chose", async () => {
    const { state } = await load({
      ...LATCHED_LOCAL,
      transcriptionMode: "void",
      useLocalWhisper: "false",
      meetingUseLocalWhisper: "false",
      meetingCloudTranscriptionMode: "void",
      noteFormattingCloudMode: "void",
    });
    assert.equal(storage.getItem("meetingTranscriptionMode"), "void", "persisted");
    assert.equal(state.meetingTranscriptionMode, "void", "reconstructed, not localized");
  ...
  await t.test("a latched BYOK profile is re-derived to its own provider", async () => {
    ...
    // `meetingTranscriptionMode` routes to Void Cloud. The reasoning side
  ...
  // dictation cleanup has since moved to Void Cloud, and an absent
  ...
  await t.test(
    ...
    async () => {
      ...
      assert.equal(state.cleanupMode, "void", "dictation cleanup untouched");
    }
  );
  ...
  await t.test(
    ...
    async () => {
      ...
      assert.equal(state.meetingTranscriptionMode, "local");
      assert.equal(state.transcriptionMode, "void", "dictation untouched");
    }
  );

  await t.test("an explicit Note Recording choice is never overwritten", async () => {
    for (const mode of ["void", "providers", "local"]) {
      const { state } = await load({
        ...LATCHED_LOCAL,
        meetingTranscriptionMode: mode,
        meetingUseLocalWhisper: String(mode === "local"),
        meetingCloudTranscriptionMode: mode === "void" ? "void" : "byok",
  ...
  await t.test("a note-formatting provider on its own is not a copy", async () => {
    ...
    assert.equal(storage.getItem("noteFormattingMode"), null);
    assert.equal(state.noteFormattingMode, "void", "store default, not derived");
    assert.equal(writes.includes("noteFormattingMode"), false);
  });
  ...
  await t.test("a copied mode-less-toggle desync follows the deliberate local flag", async () => {
    for (const mode of ["void", "providers"]) {
      const { mod, state } = await load({
        ...LATCHED_LOCAL,
        meetingTranscriptionMode: mode,
        meetingUseLocalWhisper: "true",
