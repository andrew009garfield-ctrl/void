...
const { inspectDirectVoidApiCalls } = require("../lib/ipcPolicyHeaderInventory");
...
test("all Void multipart transcription transports use policy headers", () => {
...
test("the structural inventory discovers newly added unprotected API routes", () => {
  ...
  assert.deepEqual(inspectDirectVoidApiCalls(fixture, "fixture.js"), [
...
test("every direct Void API route is policy-protected or explicitly exempt", () => {
  const calls = helperSources.flatMap(({ fileName, source: helperSource }) =>
    inspectDirectVoidApiCalls(helperSource, fileName)
  );
  assert.ok(calls.length > 0, "the helper inventory must discover Void API calls");
  ...
  assert.deepEqual(
    unclassified,
    [],
    "new direct Void routes must use policy headers or declare a non-policy reason"
  );
