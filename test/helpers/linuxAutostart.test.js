...
const ICON_THEME_SUBPATH = path.join("icons", "hicolor", "256x256", "apps", "void.png");
...
function withTmpXdgDirs(fn) {
  return async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "void-autostart-test-"));
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/home/user/Applications/Void.AppImage";
    assert.equal(resolveExecutablePath(), "/home/user/Applications/Void.AppImage");
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    fs.writeFileSync(path.join(installDir, "void"), "#!/bin/bash\n");
    withExecPath(path.join(installDir, "void-app"), () => {
      assert.equal(resolveExecutablePath(), path.join(installDir, "void"));
    });
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    const orphan = path.join(root, "opt", "void-app");
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents("/a/b/Void.AppImage", "void");
    assert.match(contents, /^Exec="\/a\/b\/Void\.AppImage" --hidden$/m);
    assert.match(contents, /^Icon=void$/m);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/apps/Void.AppImage";
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents("/a/b/Void.AppImage", null);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    const contents = buildDesktopFileContents('/home/o"neill/$HOME/`app`\\v/Void', null);
    assert.match(
      contents,
      /^Exec="\/home\/o\\"neill\/\\\$HOME\/\\`app\\`\\\\\\\\v\/Void" --hidden$/m
    );
  })
);
...
test(
  ...
  withTmpXdgDirs(async (root) => {
    ...
    setAutostartEnabled(true);
    assert.match(fs.readFileSync(getDesktopFilePath(), "utf8"), /^Icon=void$/m);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/old/path/Void.AppImage";
    setAutostartEnabled(true);

    process.env.APPIMAGE = "/new/path/Void-1.9.0.AppImage";
    assert.equal(syncAutostartEntry(), true);

    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/new\/path\/Void-1\.9\.0\.AppImage" --hidden$/m);
  })
);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/path/Void.AppImage";
    setAutostartEnabled(true);
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/new/path/Void.AppImage";
    writeEntry(
      getDesktopFilePath(),
      '[Desktop Entry]\nType=Application\nExec="/old/path/Void.AppImage"\nX-GNOME-Autostart-enabled=false\n'
    );
...
test(
  ...
  withTmpXdgDirs(async () => {
    ...
    process.env.APPIMAGE = "/installed/Void.AppImage";
    setAutostartEnabled(true);
    ...
    const contents = fs.readFileSync(getDesktopFilePath(), "utf8");
    assert.match(contents, /^Exec="\/installed\/Void\.AppImage" --hidden$/m);
  })
);
