...
test("cleanup toast localizes AWS recovery guidance and keeps fallback status quieter", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-cleanup-failure-toast-",
    mockModules: {
      "/ui/useToast": `
  ...
  const failure = {
    message:
      "AWS Bedrock is temporarily unavailable due to high demand. This is an AWS service issue, not an Void outage. Please try again in a few minutes.",
  ...
  assert.deepEqual(globalThis.__cleanupFailureToasts[0], {
    title:
      "AWS Bedrock no está disponible temporalmente debido a una alta demanda. Este es un problema del servicio de AWS, no una interrupción de Void. Vuelve a intentarlo en unos minutos.",
...
test("technical AWS details use the selected UI language", async (t) => {
  installBrowserGlobals(t);
  const vite = await createRendererServer(t, {
    cachePrefix: "void-technical-error-details-",
  });
