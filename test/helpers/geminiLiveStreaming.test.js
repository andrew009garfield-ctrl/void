...
test("url: the two Live methods are auth-disjoint and pinned per mode", () => {
  ...
  assert.equal(
    buildGeminiLiveUrl({ mode: "void", token: "auth_tokens/abc" }),
...
test("setup: the custom dictionary always ships; a language only when chosen", () => {
  ...
  assert.deepEqual(streaming.buildSetupMessage({ keyterms: ["Void", " ", ""] }), {
    setup: {
      model: "models/gemini-3.5-transcribe-live",
      generationConfig: { responseModalities: ["TEXT"] },
      inputAudioTranscription: { customVocabulary: ["Void"] },
    },
  });
...
test("connect resolves on setupComplete, not on socket open", async () => {
  await withServer(
    async ({ streaming, received }) => {
      ...
      const connected = streaming.connect({ token: "t", mode: "byok", keyterms: ["Void"] });
  ...
  await withServer(
    async ({ streaming }) => {
      ...
      assert.deepEqual(finals, [
        "The quick brown fox.",
        "The quick brown fox. Void transcription test.",
      ]);
      assert.equal(
    ...
    (socket, message) => {
      ...
      socket.send(
        JSON.stringify({
          serverContent: { inputTranscription: { text: "Void transcription test." } },
        })
      );
    }
  );
