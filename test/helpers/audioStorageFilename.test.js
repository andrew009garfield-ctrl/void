...
test("names saved audio in local time whether or not the timestamp carries a zone", (t) => {
  ...
  assert.equal(filenameFor("2026-09-10 12:00:00.000Z"), "Void-2026-09-10-05-00-00-42.webm");
  assert.equal(filenameFor("2026-09-10 12:00:00"), "Void-2026-09-10-05-00-00-42.webm");
});

test("falls back to the bare name when the timestamp is unusable", () => {
  assert.equal(filenameFor(null), "Void-42.webm");
  assert.equal(filenameFor("not a date"), "Void-42.webm");
});
