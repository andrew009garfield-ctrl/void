...
test("adds the agent name when missing, without naming other words", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(agentNameDictionaryChanges(["Alice"], "Void"), {
    add: ["Void"],
    remove: [],
  });
});
...
test("swaps the previous agent name for the new one on rename", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(agentNameDictionaryChanges(["Void", "Alice"], "Jarvis", "Void"), {
    add: ["Jarvis"],
    remove: ["Void"],
  });
});
...
test("does not ask to remove an old name the dictionary never had", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(agentNameDictionaryChanges(["Jarvis", "Alice"], "Jarvis", "Void"), {
    add: [],
    remove: [],
  });
});
...
test("never names a word outside the agent name itself", async () => {
  const { agentNameDictionaryChanges } = await load();
  const dictionary = ["Void", "Alice", "Bob", "Imported Term"];
  const { add, remove } = agentNameDictionaryChanges(dictionary, "Jarvis", "Void");
  assert.deepEqual([...add, ...remove].sort(), ["Jarvis", "Void"]);
});

test("does not add a case-variant of an agent name already in the dictionary", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(agentNameDictionaryChanges(["void", "Alice"], "Void"), {
    add: [],
    remove: [],
  });
});
...
test("does not recase an existing agent name when only capitalization changes", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(agentNameDictionaryChanges(["Void"], "void", "Void"), {
    add: [],
    remove: [],
  });
});
...
test("removes previous agent name when oldName contains surrounding whitespace", async () => {
  const { agentNameDictionaryChanges } = await load();
  assert.deepEqual(
    agentNameDictionaryChanges(["Void", "Alice"], "Jarvis", "  Void  "),
    {
      add: ["Jarvis"],
      remove: ["Void"],
    }
