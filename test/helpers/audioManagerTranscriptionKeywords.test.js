...
const DICTIONARY = "Void, Gizmo Labs";
...
test("the custom dictionary rides gpt-transcribe's keywords[] channel, legacy models' prompt", async (t) => {
  const { setSettings, createManager } = await loadAudioManager(t, {
    cachePrefix: "void-transcription-keywords-test-",
  ...
  await t.test("gpt-transcribe sends one keywords[] entry per term and no prompt", async () => {
    ...
    assert.deepEqual(requests, [
      { prompt: null, keywords: ["Void", "Gizmo Labs"], stream: "true" },
    ]);
  });
  ...
  await t.test(
    ...
    async () => {
      ...
      assert.deepEqual(requests[0].keywords, ["Void", "Gizmo Labs"]);
      assert.match(requests[0].prompt, /繁體中文/);
      assert.doesNotMatch(requests[0].prompt, /Void/, "dictionary must not be sent twice");
    }
  );
