...
test("cloud is reachable with no model", async () => {
  ...
  assert.equal(result.config.provider, "void");
});
...
test("managed mode never falls through to a stale provider when signed out", async () => {
  ...
  assert.equal(result.reachable, false);
  assert.equal(result.displayProvider, "void");
  assert.equal(result.config.provider, undefined);
});
...
const imageWired = (providerId) =>
  ["openai", "anthropic", "gemini", "void"].includes(providerId);
const panelSettings = {
  ...baseSettings,
  isSignedIn: false,
  ...
  const { config, attachScreenContext } = panel(
    {
      ...
      isSignedIn: true,
      dictationAgentMode: "void",
      dictationAgentCloudMode: "void",
      dictationAgentProvider: "void",
      dictationAgentModel: "",
    },
    { hasScreenContext: true }
  );
