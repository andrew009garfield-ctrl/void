...
test("darwin: PID events exclude current Void processes and continue during recording", async () => {
...
test("win32: current Void PIDs are filtered dynamically in JavaScript", async () => {
...
test("darwin: native snapshots exclude background speech while preserving meeting capture", async (t) => {
  ...
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "void-mac-mic-listener-"));
...
test("win32: portable native state seam handles reference counts and failures", (t) => {
  const temporaryDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "void-mic-listener-"));
  const executablePath = path.join(temporaryDirectory, "mic-listener-state-test");
  t.after(() => fs.rmSync(temporaryDirectory, { force: true, recursive: true }));
...
// dictation opens the mic from Chromium's audio service, so Void's own
