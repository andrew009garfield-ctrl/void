...
describe("modelDirUtils cache policy (#1279, #1399)", () => {
  ...
  function createRedirectedWindowsFixture() {
    ...
    const legacyRoot = path.join(home, ".cache", "void");
    const redirectedProfile = path.join(tempRoot, "RedirectedUsers", "stan");
    const redirectedRoot = path.join(redirectedProfile, ".cache", "void");
  ...
  beforeEach(() => {
    ...
    delete process.env.VOID_CACHE_ROOT;
  ...
  it("pathHasProblematicChars flags non-ASCII and spaces", () => {
    ...
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Anton\\.cache\\void"), false);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Антон\\.cache\\void"), true);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\詩涵\\.cache\\void"), true);
    assert.strictEqual(pathHasProblematicChars("C:\\Users\\Stan Shih\\.cache\\void"), true);
  });

  it("honors VOID_CACHE_ROOT when ASCII-safe", () => {
    setPlatform("win32");
    const override = path.join(tempRoot, "ascii-cache");
    process.env.VOID_CACHE_ROOT = override;
  ...
  it("gives VOID_CACHE_ROOT precedence over an ASCII-safe Windows home", () => {
    setPlatform("win32");
    const home = path.join(tempRoot, "Users", "stan");
  ...
  it("honors an absolute XDG_CACHE_HOME on Linux", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(xdgCacheHome, "void"));
  });

  it("gives VOID_CACHE_ROOT precedence over XDG_CACHE_HOME", () => {
    setPlatform("linux");
    const home = path.join(tempRoot, "home", "stan");
    const override = path.join(tempRoot, "custom-cache");
    process.env.VOID_CACHE_ROOT = override;
  ...
  it("keeps the home cache when XDG_CACHE_HOME is relative", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(home, ".cache", "void"));
  });
  ...
  it("honors VOID_CACHE_ROOT on macOS", () => {
    setPlatform("darwin");
    const home = path.join(tempRoot, "Users", "stan");
    const override = path.join(tempRoot, "custom-cache");
    process.env.VOID_CACHE_ROOT = override;
  ...
  it("falls back to ProgramData when redirected USERPROFILE is unsafe", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(programData, "Void", "cache"));
  });
  ...
  it("skips an unsafe ProgramData fallback", () => {
    ...
    assert.strictEqual(getCacheRoot(), path.join(systemDrive, "Void", "cache"));
  });
  ...
  it("falls back to ProgramData cache when home cache path is non-ASCII", () => {
    ...
    assert.strictEqual(root, path.join(programData, "Void", "cache"));
  ...
  it("never overwrites models already present at the safe root", () => {
    ...
    const home = path.join(tempRoot, "使用者", "詩涵");
    const legacyDir = path.join(home, ".cache", "void", "whisper-models");
    fs.mkdirSync(legacyDir, { recursive: true });
    fs.writeFileSync(path.join(legacyDir, "ggml-base.bin"), "old");

    const newDir = path.join(programData, "Void", "cache", "whisper-models");
