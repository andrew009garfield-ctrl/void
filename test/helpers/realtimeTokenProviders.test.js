...
test("openai cloud mints per-stream client secrets and validates the dual pair", async () => {
  ...
  const options = { mode: "void", model: "gpt-4o-mini-transcribe", language: "en" };
...
test('wire bodies: dictation posts the bare {"streams":1}; meetings post model+language+streams', async () => {
  ...
  await fetchRealtimeTokenForProvider("openai-realtime", cloudDeps, { mode: "void" });
  // Meeting with system audio.
  await fetchRealtimeTokenForProvider(
    "openai-realtime",
    cloudDeps,
    { mode: "void", model: "gpt-4o-mini-transcribe", language: "en" },
    { streams: 2 }
  );
  ...
  await fetchRealtimeTokenForProvider(
    "openai-realtime",
    cloudDeps,
    { mode: "void", model: "gpt-4o-mini-transcribe", language: undefined },
    { streams: 1 }
  );

  // Gemini's endpoint reads nothing from the request; it must post a bare body.
  await fetchRealtimeTokenForProvider("gemini-realtime", cloudDeps, { mode: "void" });
