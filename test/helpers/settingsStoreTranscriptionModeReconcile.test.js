...
test("startup repairs transcription routing that disagrees with the selected mode", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-transcription-mode-reconcile-test-",
  });
  ...
  for (const mode of ["void", "providers", "self-hosted", "enterprise", "nonsense"]) {
  ...
  await t.test("the local rule leaves the cloud mode alone", async () => {
    ...
    assert.equal(storage.getItem("cloudTranscriptionMode"), "void", "untouched");
  });

  for (const mode of ["void", "enterprise", "nonsense"]) {
    await t.test(`a "${mode}" mode never triggers the cloud rule`, async () => {
      ...
      assert.equal(storage.getItem("cloudTranscriptionMode"), "void");
    });
  }
