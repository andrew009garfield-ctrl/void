...
let userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-cli-dict-"));
...
function createBridge(t) {
  userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-cli-dict-"));
  let db;
  try {
...
test("GET /v1/dictionary/list returns the stored words", (t) => {
  ...
  ctx.db.setDictionary(["Void", "Alice"]);
  const result = call(ctx.bridge, "GET", "/v1/dictionary/list");
  assert.deepEqual(result.data, ["Void", "Alice"]);
});
...
test("POST /v1/dictionary/update bulk-adds words that survive a reload", (t) => {
  ...
  assert.deepEqual(ctx.db.getDictionary(), ["Void", ...contacts]);
...
test("POST /v1/dictionary/update broadcasts the new list to renderers", async (t) => {
  ...
  assert.deepEqual(ctx.broadcasts[0].payload, ["Void", "Alice"]);
});
