...
test("win32 query-string URLs go to shell.openExternal — explorer.exe opens File Explorer instead of the browser for them", async () => {
  ...
  const oauthUrl =
    "https://auth.void.com/api/desktop-signin/google?callbackURL=void%3A%2F%2Fauth";
...
test("win32 fragment URLs go to shell.openExternal — explorer.exe mishandles them too", async () => {
  ...
  await openExternalUrl("https://docs.void.com/changelog#windows");

  assert.equal(spawnCalls.length, 0);
  assert.deepEqual(openExternalCalls, ["https://docs.void.com/changelog#windows"]);
});
