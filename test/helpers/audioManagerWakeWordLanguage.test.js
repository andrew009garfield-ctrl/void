...
async function loadAudioManager(t) {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-wake-language-test-",
    mockModules: {
      "/utils/logger":
...
test("cloud auto-language routing uses detected speech before the UI language", async (t) => {
  ...
  const englishResult = await createManager().processWithVoidCloud(audioBlob);
  ...
  const italianResult = await createManager().processWithVoidCloud(audioBlob);
...
test("cloud transcription returns the occurrence time sent with analytics", async (t) => {
  ...
  const result = await createManager().processWithVoidCloud(audioBlob, {
    analyticsOccurredAt,
  });
...
test("cloud auto-language stripping uses the same detected Arabic as routing", async (t) => {
  ...
  await manager.processWithVoidCloud(audioBlob);
...
test("a name inside a snippet trigger does not route to the agent", async (t) => {
  ...
  const result = await createManager().processWithVoidCloud(snippetAudioBlob);
...
test("the banked agent command drops the address, not the snippet trigger", async (t) => {
  ...
  const manager = createBankingManager();
  await manager.processWithVoidCloud(snippetAudioBlob);
