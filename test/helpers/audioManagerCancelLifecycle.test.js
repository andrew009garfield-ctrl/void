...
async function loadManagerClass(t) {
  const { AudioManager, window } = await loadAudioManager(t, {
    cachePrefix: "void-cancel-lifecycle-test-",
...
async function loadCancelGuardManagerClass(t) {
  const { AudioManager, window } = await loadAudioManager(t, {
    cachePrefix: "void-streaming-leak-test-",
    ...
    mockModules: {
      ...
      `,
      // processWithVoidCloud's "agent" route walks through
...
function createStreamingLeakManager(AudioManager) {
  ...
  const manager = Object.assign(Object.create(AudioManager.prototype), {
    ...
    // Real processWithVoidCloud runs; these three would otherwise need
...
test("a cancelled batch pipeline cannot clear newer streaming finalization", async (t) => {
  ...
  manager.processWithVoidCloud = () => oldTranscription.promise;
  ...
  oldTranscription.resolve({ success: true, text: "old", source: "void", timings: {} });
  await oldRun;
...
// Finding 2 (round-1 review override): processWithVoidCloud's catch
...
test(
  ...
  async (t) => {
    ...
    const result = await manager.processWithVoidCloud(
      {
        size: 1024,
        type: "audio/webm",
        arrayBuffer: async () => new ArrayBuffer(8),
