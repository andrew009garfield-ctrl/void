...
test("fresh Chinese browser locale survives settings hydration", async (t) => {
  ...
  installBrowserGlobals(t, {
    initialStorage: {
      customDictionary: JSON.stringify(["Void"]),
    },
    window: {
      electronAPI: {
        getDictionary: async () => ["Void"],
        getUiLanguage: async () => "",
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-ui-language-startup-test-",
  });
...
test("main locale fallback remains implicit and yields to an explicit preference", (t) => {
  const userDataDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "void-ui-language-"));
