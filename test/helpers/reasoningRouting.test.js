...
test("void cloud mode maps to the void mode", async () => {
  const { deriveReasoningMode } = await load();
  assert.equal(deriveReasoningMode("void", "corti"), "void");
});
...
test("fan-out with partial settings only mirrors the provided routing fields", async () => {
  ...
  const {
    ...
    chatIntelligence,
    dictationTranslation,
  } = buildReasoningScopePatches({ useCleanupModel: true }, "void");

  assert.equal(dictationCleanup.useCleanupModel, true);
  assert.equal(dictationCleanup.cleanupMode, "void");
  assert.equal("cleanupProvider" in dictationCleanup, false);

  for (const scope of [noteFormatting, dictationAgent, chatIntelligence, dictationTranslation]) {
    assert.deepEqual(scope, { mode: "void" });
  }
});

const VOID_REASONING = { useCleanupModel: true, cleanupCloudMode: "void" };
...
test("us data region routes reasoning to void cloud, transcription stays corti", async () => {
  const { buildCortiOnboardingPayloads } = await load();
  const { transcription, reasoning } = buildCortiOnboardingPayloads(
    { id: "corti", models: [{ id: "corti-transcribe" }] },
    { id: "corti", models: [{ id: "corti-s1-instant" }] },
  ...
  assert.deepEqual(reasoning, VOID_REASONING);
  assert.equal(transcription.cloudTranscriptionProvider, "corti");
});

test("eu region without an api key routes reasoning to void cloud", async () => {
  const { buildCortiOnboardingPayloads } = await load();
  const { reasoning } = buildCortiOnboardingPayloads(
    { id: "corti", models: [{ id: "corti-transcribe" }] },
    { id: "corti", models: [{ id: "corti-s1-instant" }] },
...
test("undefined data region routes reasoning to void cloud", async () => {
  const { buildCortiOnboardingPayloads } = await load();
  const { reasoning } = buildCortiOnboardingPayloads(
    { id: "corti", models: [{ id: "corti-transcribe" }] },
    { id: "corti", models: [{ id: "corti-s1-instant" }] },
...
test("missing corti reasoning provider routes reasoning to void cloud", async () => {
  const { buildCortiOnboardingPayloads } = await load();
  const { transcription, reasoning } = buildCortiOnboardingPayloads(
    { id: "corti", models: [{ id: "corti-transcribe" }] },
    undefined,
...
test("corti reasoning provider with empty models routes reasoning to void cloud", async () => {
  const { buildCortiOnboardingPayloads } = await load();
  const { reasoning } = buildCortiOnboardingPayloads(
    { id: "corti", models: [{ id: "corti-transcribe" }] },
    { id: "corti", models: [] },
