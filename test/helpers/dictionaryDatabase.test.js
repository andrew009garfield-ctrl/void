...
let userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-dict-db-"));
...
function createDb(t) {
  userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-dict-db-"));
  try {
...
test("setDictionary replaces the full dictionary (why a stale cache wipe was destructive)", (t) => {
  ...
  db.setDictionary(["Void", "Alice", "Bob"]);
  assert.deepEqual(db.getDictionary(), ["Void", "Alice", "Bob"]);

  // Writing only the cached subset is exactly what startup used to do.
  db.setDictionary(["Void"]);
  assert.deepEqual(db.getDictionary(), ["Void"]);
});
...
test("startup reconcile preserves DB words that a stale renderer cache omitted (#1295)", async (t) => {
  ...
  db.setDictionary(["Void", "Alice", "Bob", "Imported Term"]);
  const staleCache = ["Void"];
  ...
  assert.deepEqual(db.getDictionary(), ["Void", "Alice", "Bob", "Imported Term"]);
});
...
test("cloud pull upserts remotes without deleting local-only pending rows", (t) => {
  ...
  assert.deepEqual(db.getDictionary().sort(), ["Alice", "Carol", "Void"].sort());
...
test("repeated cloud upsert does not duplicate words", (t) => {
  ...
  assert.deepEqual(db.getDictionary().sort(), ["Delta", "Void"].sort());
});

test("default Void row is neither removed nor duplicated by cloud upsert", (t) => {
  const db = createDb(t);
  if (!db) return;
  ...
  const voidRows = db.db
    .prepare("SELECT * FROM custom_dictionary WHERE lower(word) = 'void'")
    .all();
  assert.equal(voidRows.length, 1);
  assert.equal(voidRows[0].cloud_id, "cloud-ow");
...
test("intentional removal tombstones synced rows and hard-deletes unsynced ones", (t) => {
  ...
  db.setDictionary(["Void", "Temp", "Synced"]);
...
test("adding words leaves every other row untouched", (t) => {
  ...
  assert.deepEqual(db.getDictionary(), ["Void", "Alice", "Bob", "Carol"]);
});
...
test("a caller holding a stale one-word view cannot delete anything (#1295)", (t) => {
  ...
  db.applyDictionaryChanges({ add: ["Void"] });
...
test("an edit is a remove plus an add in one transaction", (t) => {
  ...
  db.setDictionary(["Void", "Kubernets", "Alice"]);
  db.applyDictionaryChanges({ add: ["Kubernetes"], remove: ["Kubernets"] });

  assert.deepEqual(db.getDictionary(), ["Void", "Alice", "Kubernetes"]);
});
...
test("an empty delta touches nothing", (t) => {
  ...
  assert.deepEqual(db.getDictionary(), ["Void", "Alice"]);
});
