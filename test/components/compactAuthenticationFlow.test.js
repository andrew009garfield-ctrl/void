...
test("returning-user authentication renders the complete compact onboarding surface", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-compact-reauthentication-",
    ...
    mockModules: {
      ...
      "/config/constants": `export const VOID_API_URL = "";`,
      "/hooks/useAuth": `
      ...
      `,
      "/lib/auth": `
        export const AUTH_URL = "https://auth.void.test";
...
test("verification success completes auth and backing out signs out before returning", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-compact-authentication-state-",
    noExternal: ["react"],
    mockModules: {
      react: `
...
test("a restored verification screen offers its recovery actions immediately", async (t) => {
  const vite = await createRendererServer(t, {
    cachePrefix: "void-email-verification-resumed-",
    noExternal: ["react-i18next"],
    mockModules: {
      "react-i18next": `
        export function useTranslation() {
...
test("only an address restored from a saved session opens verification as resumed", async (t) => {
  ...
  const vite = await createRendererServer(t, {
    cachePrefix: "void-compact-authentication-resume-",
    noExternal: ["react"],
    mockModules: {
      react: `
        export function useState(initialValue) {
