...
test("shared field primitives inherit unless a consumer declares its content direction", async (t) => {
  const vite = await createRendererServer(t, {
    cachePrefix: "void-field-direction-primitives-",
  });
  ...
  const inherited = renderToStaticMarkup(
    React.createElement(Input, { value: "مرحبا Void 2.0", readOnly: true })
  );
  assert.doesNotMatch(inherited, /\sdir=/);
  assert.match(inherited, /value="مرحبا Void 2.0"/);
  ...
  const prose = renderToStaticMarkup(
    React.createElement(Textarea, {
      dir: "auto",
      value: "مرحبا Void 2.0",
      readOnly: true,
    })
  );
  assert.match(prose, /\sdir="auto"/);
  assert.match(prose, />مرحبا Void 2.0<\/textarea>/);
});
