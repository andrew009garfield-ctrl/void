...
const offersWith = async (overrides) => {
  ...
  return eligibleGpuOffers({
    ...
    dictationAgentMode: "void",
    agentAllowedByPolicy: true,
    ...overrides,
  });
};

test("cloud cleanup (void default) is not eligible for the intelligence GPU offer", async () => {
  const offers = await offersWith({ useCleanupModel: true, cleanupMode: "void" });

  assert.equal(offers.intelligence, null);
});

test("cloud cleanup modes are never eligible even with cleanup enabled", async () => {
  for (const cleanupMode of ["void", "providers", "self-hosted", "enterprise"]) {
...
test("a disabled or cloud-mode dictation agent is not eligible", async () => {
  ...
  for (const dictationAgentMode of ["void", "providers", "self-hosted", "enterprise"]) {
