...
test("managed Void Cloud does not advertise live transcription preview", async () => {
  const { supportsLiveTranscriptionPreview } = await load();

  assert.equal(supportsLiveTranscriptionPreview("void", true), false);
});
...
test("streaming preview runs for BYOK only, even if a stale setting remains enabled", async () => {
  ...
  assert.equal(shouldShowByokStreamingPreview(true, "void"), false);
