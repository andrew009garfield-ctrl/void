...
function resolveAppChannel() {
  const rawChannel = (process.env.VOID_CHANNEL || process.env.VITE_VOID_CHANNEL || "")
    .trim()
    .toLowerCase();
...
process.env.VOID_CHANNEL = APP_CHANNEL;
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `Void-${APP_CHANNEL}`);
...
if (process.platform === "linux") {
  app.setDesktopName("void.desktop");
}
...
function getOAuthProtocol() {
  const fromEnv = (process.env.VITE_VOID_PROTOCOL || process.env.VOID_PROTOCOL || "")
    .trim()
    .toLowerCase();
...
// True source of truth for whether void:// resolves on Linux — the same
...
const protocolRegistered = registerVoidProtocol() || isOAuthSchemeRegistered();
...
if (process.platform === "darwin" && app.getName() !== "Void") {
  app.setName("Void");
}
...
const { applyVoidOriginHeader: applyVoidOriginHeader } = require("./src/helpers/sessionHeaders");
...
function parseAuthBridgePort() {
  const raw = (process.env.VOID_AUTH_BRIDGE_PORT || "").trim();
...
function startAuthBridgeServer() {
  ...
  authBridgeServer = http.createServer(async (req, res) => {
    ...
    res.end(
      "<html><body><h3>Void sign-in complete.</h3><p>You can close this tab.</p></body></html>"
    );
  });
...
async function startApp() {
  ...
  applyVoidOriginHeader(session.defaultSession);
