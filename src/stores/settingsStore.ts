...
const TRANSCRIPTION_POLICY_CATALOG = {
  modes: ["void", "providers", "local", "self-hosted", "enterprise"] as const,
...
const MEETING_TRANSCRIPTION_POLICY_CATALOG = {
  ...
  modes: ["void", "providers", "local"] as const,
...
// so nothing is copied and the upload context falls through to its Void
// Cloud defaults.
...
// (the `isVoidCloud` test), so routing can disagree with what the user sees
...
function reconcileTranscriptionRouting(): void {
  ...
  for (const keys of TRANSCRIPTION_ROUTING_KEYS) {
    ...
    if (
      ...
      localStorage.getItem(keys.cloudMode) === "void"
    ) {
...
// Void Cloud, so every mode is reconstructed from the snapshot the copy
...
// `noteFormattingMode` does have one: an absent mode reads "void", but
// selectIsCloudNoteFormattingMode also requires cloudMode "void", and the
...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  cloudTranscriptionMode: readString("cloudTranscriptionMode", "void"),
  cleanupCloudMode: readString("cleanupCloudMode", "void"),
  ...
  transcriptionMode: (() => {
    const v = readString("transcriptionMode", "void");
    if (v === "void" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "void" as InferenceMode;
  })(),
  ...
  cleanupMode: (() => {
    const v = readString("cleanupMode", "void");
    if (
      v === "void" ||
      v === "providers" ||
      v === "local" ||
  ...
  meetingTranscriptionMode: (() => {
    const v = readString("meetingTranscriptionMode", "void");
    if (v === "void" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "void" as InferenceMode;
  })(),
  ...
  uploadTranscriptionMode: (() => {
    const v = readString("uploadTranscriptionMode", "void");
    if (v === "void" || v === "providers" || v === "local" || v === "self-hosted") return v;
    return "void" as InferenceMode;
  })(),
  ...
  noteFormattingMode: (() => {
    const v = readString("noteFormattingMode", "void");
    if (
      v === "void" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationMode: (() => {
    const v = readString("translationMode", "void");
    if (
      v === "void" ||
      v === "providers" ||
      v === "local" ||
  ...
  translationCloudMode: readString("translationCloudMode", "void"),
  ...
  chatAgentCloudMode: readString("chatAgentCloudMode", "void"),
  chatAgentMode: (() => {
    const v = readString("chatAgentMode", "void");
    if (
      v === "void" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentMode: (() => {
    const v = readString("dictationAgentMode", "void");
    if (
      v === "void" ||
      v === "providers" ||
      v === "local" ||
  ...
  dictationAgentCloudMode: readString("dictationAgentCloudMode", "void"),
  ...
  dictationAgentVisionCloudMode: readString("dictationAgentVisionCloudMode", "void"),
  ...
  setCloudTranscriptionForAllScopes: (settings: Partial<TranscriptionSettings>) => {
    ...
    // scope — otherwise the UI keeps showing the previous mode (e.g. Void
  ...
  setCloudReasoningForAllScopes: (settings) => {
    ...
    // collapses everything non-byok to "void", which would misroute
    ...
    // ever hold real cloud routings ("void"/"byok").
...
export function selectPolicyEffectiveSettings(
  ...
  for (const keys of TRANSCRIPTION_CONTEXT_KEYS) {
    ...
    writable[keys.cloudMode] = selection.mode === "void" ? "void" : "byok";
  ...
  for (const scope of Object.keys(INFERENCE_SCOPES) as InferenceScope[]) {
    ...
    if (definition.storeKeys.cloudMode) {
      writable[definition.storeKeys.cloudMode] =
        selection.mode === "void" ? "void" : "byok";
    }

    let provider = selection.provider;
    if (selection.mode === "void") provider = "void";
      ...
      if (!localStorage.getItem("enterpriseSetupMode")) {
        ...
        const hasChosenProvider =
          Object.values(INFERENCE_SCOPES).some((scope) => {
            ...
            return Boolean(stored) && stored !== "void";
          }) ||
          Boolean(
