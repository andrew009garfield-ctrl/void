...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  const isVoidCloud =
    isSignedIn && cloudTranscriptionMode === "void" && !useLocalWhisper;

  // Mode detection
  const isSelfHosted = transcriptionMode === "self-hosted" && !useLocalWhisper;
  const isByok = !useLocalWhisper && !isVoidCloud;
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      // Cloud (Void) — user is always signed in here
  ...
  const getActiveModelLabel = (): string => {
    if (isVoidCloud) return t("notes.upload.voidCloud");
    if (useLocalWhisper) {
  ...
  const handleTranscribe = async () => {
    ...
    const useChunkProgress = isVoidCloud && isLargeFile;
  ...
  const switchToCloud = () => {
    setUploadTranscriptionMode("void");
    setUploadCloudTranscriptionMode("void");
    setUploadUseLocalWhisper(false);
  };

  const getTranscribingLabel = (): string => {
    if (isVoidCloud) return t("notes.upload.transcribingCloud");
      ...
      >
        ...
        {(state === "idle" || state === "selected") && (
          <div className="max-w-[320px] mx-auto mt-4">
            ...
            {diarizationEnabled && isVoidCloud && (
              <p className="text-[10px] text-foreground/45 mt-1.5">
...
function SelectedView({
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {isLargeFile && !requiresUpgrade && !fileTooLarge && isVoidCloud && (
