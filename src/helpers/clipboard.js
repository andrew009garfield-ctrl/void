...
class ClipboardManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      proc.on("close", (code) => {
        ...
        if (code === 0) {
          ...
          reject(new Error("portal symbolic keyboard input unavailable"));
        } else {
          reject(
            new Error(
  ...
  async pasteLinux(originalClipboard, options = {}) {
        ...
        const tryPortalPaste = async () => {
          try {
            ...
            if (portalError?.message === "portal-denied") {
              ...
              portalError?.message === "portal support not compiled in" ||
    ...
    if (isTerminalTarget && xdotoolExists && !isWayland) {
      debugLogger.debug(
  ...
  showAccessibilityDialog(testError) {
    ...
    if (isStuckPermission) {
      dialogMessage = `🔒 Void needs Accessibility permissions, but it looks like you may have OLD PERMISSIONS from a previous version.

❗ COMMON ISSUE: If you've rebuilt/reinstalled Void, the old permissions may be "stuck" and preventing new ones.

🔧 To fix this:
1. Open System Settings → Privacy & Security → Accessibility
2. Look for ANY old "Void" entries and REMOVE them (click the - button)
3. Also remove any entries that say "Electron" or have unclear names
4. Click the + button and manually add the NEW Void app
...
Would you like to open System Settings now?`;
      ...
      dialogMessage = `🔒 Void needs Accessibility permissions to paste text into other applications.
...
4. Add Void to the list and check the box
5. Restart Void
...
Would you like to open System Settings now?`;
  ...
  checkPasteTools() {
    ...
    if (!isWayland) {
      ...
      if (portalKeysymAvailable) tools.push("portal");
      if (canUseXdotool && this.commandExists("xdotool")) tools.push("xdotool");
      if (canUseYdotool) tools.push("ydotool");
    }
    ...
    return {
      ...
      available,
      method,
      requiresPermission: method === "portal",
      isWayland,
      xwaylandAvailable,
      hasNativeBinary,
      hasUinput,
