...
`;
  ...
  async processAudio(audioBlob, metadata = {}, processingPipeline = null) {
    ...
    try {
      ...
      const isVoidCloudMode = !useLocalWhisper && cloudTranscriptionMode === "void";
      const useCloud = isVoidCloudMode && isSignedIn;
      logger.debug(
      ...
      let result;
      let activeModel;
      // Managed enterprise STT outranks the local and Void Cloud lanes,
      ...
      } else if (isVoidCloudMode) {
        if (!isSignedIn) {
          const err = new Error(
            "Void Cloud requires sign-in. Please sign in again or switch to BYOK mode."
          );
          err.code = "AUTH_REQUIRED";
          err.messageKey = "hooks.audioRecording.errorDescriptions.sessionExpired";
          throw err;
        }
        activeModel = "void-cloud";
        result = await this.processWithVoidCloud(audioBlob, metadata, wasCancelled);
      } else {
  ...
  async processAgentCommand(text, model, agentName, config, wasCancelled = neverCancelled) {
    ...
    const completionMarker = `__VOID_SELECTION_COMPLETE_${crypto.randomUUID()}__`;
  ...
  async runTranslationChain({ text, settings, agentName, route, cleanup }) {
    ...
    try {
      const chainResult = await executeTranslationChain({
        ...
        translateIsCloud: route.config?.provider === "void",
  ...
  async processTranscriptionCore(text, source, wasCancelled = neverCancelled) {
    ...
    const agentName =
      typeof window !== "undefined" && window.localStorage ? getAgentName() : "Void";
    if (
      !cleanupReachable &&
      !agentReachable &&
  ...
  async processWithVoidCloud(audioBlob, metadata = {}, wasCancelled = neverCancelled) {
    if (!navigator.onLine) {
    ...
    const cleanupCloudMode = settings.cleanupCloudMode || "void";
    if (
      (settings.useCleanupModel && cleanupCloudMode === "void") ||
    ...
    if (processedText) {
      ...
      try {
        ...
        } else if (route.kind === "cleanup" && cleanupCloudMode === "void") {
          const reasonResult = await withSessionRefresh(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) err.code = "INVALID_KEY";
          ...
          // The user's own provider rate-limited the request — not an Void plan limit
  ...
  shouldUseStreaming(isSignedInOverride) {
    ...
    // Corti (BYOK) streams over its own WSS — independent of Void Cloud.
    if (s.cloudTranscriptionProvider === "corti" && s.cloudTranscriptionMode === "byok") {
      return !!(s.cortiClientId && s.cortiClientSecret);
    }

    // Tinfoil realtime streams without an Void account.
    ...
    // The managed-cloud bootstrap only controls Void Cloud. A user's
    // BYOK realtime model must not be downgraded because managed dictation is
    // configured for batch processing.
    if (s.cloudTranscriptionMode === "void" && this.sttConfig?.dictation?.mode === "batch") {
      return false;
    }
    ...
    if (REALTIME_MODELS.has(s.cloudTranscriptionModel)) {
      ...
      if (s.cloudTranscriptionMode === "void") return !!(isSignedInOverride ?? s.isSignedIn);
      return false;
    }

    if (s.cloudTranscriptionMode !== "void" || !(isSignedInOverride ?? s.isSignedIn)) {
      return false;
    }
  ...
  async startStreamingRecording(forceDefaultMic = false) {
    ...
    try {
      ...
      if (error.name === "NotAllowedError" || error.name === "PermissionDeniedError") {
        ...
        errorDescription =
          "Your Void Cloud session is unavailable. Please sign in again from Settings.";
  ...
  async _finalizeStreamingRecording(sessionId) {
    ...
    if (finalText) {
      ...
      const cleanupCloudMode = stSettings.cleanupCloudMode || "void";
    ...
    if (!finalText && durationSeconds > 2 && fallbackBlob?.size > 0) {
      ...
      if (target === "skip") {
        logger.warn(
          "Skipping batch fallback: Void Cloud session signed out",
          {},
          "streaming"
        );
      } else {
        ...
        try {
          ...
          const batchResult =
            target === "cloud"
              ? await this.processWithVoidCloud(
                  fallbackBlob,
                  {
                    durationSeconds,
                    analyticsOccurredAt: analyticsOccurredAt.toISOString(),
            ...
            await withSessionRefresh(async () => {
              const res = await window.electronAPI.cloudStreamingUsage(
                ...
                durationSeconds ?? 0,
                {
                  ...
                  // "void_cloud" whatever actually transcribed the audio,
