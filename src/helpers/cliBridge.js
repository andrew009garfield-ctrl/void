...
function getBridgeFilePath() {
  return path.join(os.homedir(), ".void", "cli-bridge.json");
}
...
class CliBridge {
  ...
  _buildRouteTable() {
    ...
    const requireLocalModel = (models, requested) => {
      ...
      if (!match) {
        const err = new Error(
          requested
            ...
            : "No local transcription model is selected. Choose one in Void under Settings → Transcription, or pass 'model'."
        );
        err.code = "VALIDATION";
        throw err;
      }
      if (!match.downloaded) {
        const err = new Error(
          `Model '${match.model}' is not downloaded. Open Void and download it under Settings → Transcription.`
        );
        err.code = "VALIDATION";
        throw err;
      }
