...
export function resolveDictationAgentVisionInference(settings, { isSignedIn = false } = {}) {
  ...
  const isCloud = isSignedIn && mode === "void" && resolved.cloudMode === "void";
 ...
 * Void Cloud, which vision-routes server-side.
 *
...
export function resolveChatStreamingInference(
  ...
  const isCloud = !!settings.isSignedIn && config.mode === "void";
  const vision =
