...
function convertToWav(inputPath, outputPath, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!ffmpegPath) {
      reject(
        new Error(
          "FFmpeg not found - the bundled FFmpeg is missing from this install and no system FFmpeg was found on PATH; reinstalling Void should fix this"
        )
      );
      return;
    }
...
async function mergeAudioSegments(segments) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "void-audio-merge-"));
