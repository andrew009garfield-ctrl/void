...
function getAsciiSafeFallbackRoot() {
  const candidates = [
    path.join(process.env.ProgramData || "C:\\ProgramData", "Void", "cache"),
    path.join(process.env.SystemDrive || "C:", "Void", "cache"),
  ];
...
function getPreferredCacheRoot(homeCache) {
  if (process.env.VOID_CACHE_ROOT) {
    return process.env.VOID_CACHE_ROOT;
  }

  if (process.platform === "win32") {
    const redirectedProfile = process.env.USERPROFILE;
    if (redirectedProfile && path.isAbsolute(redirectedProfile)) {
      return path.join(redirectedProfile, ".cache", "void");
    }
  }
  ...
  if (process.platform === "linux") {
    ...
    if (xdgCacheHome && path.isAbsolute(xdgCacheHome)) {
      return path.join(xdgCacheHome, "void");
    }
  }
...
function getCacheRoot() {
  ...
  const homeCache = path.join(homeDir, ".cache", "void");
