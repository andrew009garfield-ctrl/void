---
name: void-cli
description: Use this skill whenever the user wants to operate on Void notes, folders, transcriptions, or audio from a terminal or shell. The Void CLI (`void` binary, npm package `@void/cli`) talks to either the local desktop app or the cloud API and exposes every operation needed for managing notes, folders, transcriptions, audio, plus auth and config. Trigger this skill when the user mentions "void cli", running shell commands against Void, automating note workflows, cleaning up a transcription, building agent integrations against Void, or scripting any Void operation — even if they don't say "CLI" explicitly.
---

# Void CLI

Use this reference when running the `void` command-line tool. The CLI is a single binary that operates against either the local desktop app (via a loopback HTTP bridge) or the cloud REST API. The same command works against both backends.
...
Requires Node.js 20 or later. Verify with `void --version`. If the user reports `command not found`, ensure their npm global bin is on `$PATH`.
...
| **remote** | `https://api.void.com/api/v1`               | Desktop is closed, or running on a different machine, or the user wants cloud-side semantics. |
...
2. The `VOID_BACKEND` environment variable (`local`, `remote`, or `auto`)
3. The `backend` key in `~/.void/cli-config.json`
...
When the desktop app starts, it writes `{version, port, token}` to `~/.void/cli-bridge.json` with mode `0600`. The CLI reads it automatically. If the file is missing or stale, local is treated as unavailable.
...
```bash
void auth login    # prompts for the key, stores it 0600 in ~/.void/cli-config.json
void auth status   # confirm it works
void auth logout   # clear it
```
...
Noun-verb syntax: `void <noun> <verb>`. Same convention as `gh`, `kubectl`, `aws`, `stripe`.

### Notes

```bash
void notes list [--folder <id>] [--limit N] [--format json|table]
void notes get <id> [--format json|markdown]
void notes create --content <text> | --content-file <path>
                        [--title <t>] [--folder <id>]
void notes update <id> [--content <t>] [--folder <id>] [--title <t>]
void notes delete <id> [--dry-run] [--format json]
void notes search <query> [--limit N] [--format json|table]
```

### Folders

```bash
void folders list [--format json|table]
void folders create --name <name> [--sort-order <n>]
```
...
```bash
void transcriptions list [--limit N] [--format json|table]
void transcriptions get <id> [--format json|text]
void transcriptions delete <id> [--dry-run] [--format json]
```
...
```bash
void audio delete <transcription-id> [--format json]
```
...
```bash
void auth login [--api-key <key>]   # prompts on stdin if --api-key omitted
void auth status [--format json]
void auth logout
```

`auth status` reads the stored config and reports whether a key is configured — it does **not** make a network call. To verify the key actually works, use `void doctor`.

### Config

```bash
void config get [--format json]
void config set backend auto|local|remote
void config set api-base https://api.void.com
```

Only `backend` and `api-base` are settable via `config set`. The API key is managed through `auth login`/`auth logout`. `api-base` is overridable for self-hosted or staging deployments (default: production cloud). The `VOID_API_BASE` env var also overrides it for a single invocation.

### Doctor

```bash
void doctor [--format json]
```
...
```bash
void notes list --limit 100 --format json | \
  jq -r '.[] | select(.title | contains("draft")) | .id' | \
  while read id; do
    void notes delete "$id"
  done
```
...
```bash
void notes search "quarterly budget" --format json | jq '.[].id'
```
...
| ------------------------------- | --------------------------------------- | ------------------------------------------------ |
| `~/.void/cli-bridge.json` | The desktop app at startup              | `{version, port, token}` for the loopback bridge |
| `~/.void/cli-config.json` | The CLI's `auth login` and `config set` | `{backend, apiBase, apiKey}`                     |
...
| ----------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------- |
| `Backend unreachable` (exit 2) on every command | Desktop closed and no API key                                 | Start desktop, or `void auth login`          |
...
| Config file readable by other users             | File created or edited outside the CLI                        | `chmod 0600 ~/.void/cli-config.json`         |
