---
name: void-api
description: Use this skill when building integrations with the Void REST API, calling Void endpoints, managing notes/folders/transcriptions programmatically, accessing team-space content with workspace keys, or connecting to the Void MCP server. Covers authentication, all V1 endpoints, spaces, pagination, rate limits, error handling, and the remote MCP server.
---

# Void API v1

Use this reference when making requests to the Void REST API. All endpoints are under the V1 path and require API key authentication.
...
```bash
claude mcp add void --transport http https://mcp.void.com/mcp \
...
curl -H "Authorization: Bearer owk_live_YOUR_KEY" \
  "https://api.void.com/api/v1/notes/list?limit=10"
```
...
curl -X POST \
  ...
  https://api.void.com/api/v1/notes/create
```
...
curl -X POST \
  ...
  https://api.void.com/api/v1/notes/search
```
...
while true; do
  response=$(curl -s -H "Authorization: Bearer owk_live_YOUR_KEY" \
    "https://api.void.com/api/v1/notes/list?limit=100&cursor=${cursor}")
...
```bash
curl -H "Authorization: Bearer owk_live_YOUR_KEY" \
  https://api.void.com/api/v1/usage
```
...
curl -H "Authorization: Bearer ow_wks_live_YOUR_KEY" \
  https://api.void.com/api/v1/spaces/list

# 2. List notes in a space (space_id is required for workspace keys)
curl -H "Authorization: Bearer ow_wks_live_YOUR_KEY" \
  "https://api.void.com/api/v1/notes/list?space_id=SPACE_UUID&limit=50"
