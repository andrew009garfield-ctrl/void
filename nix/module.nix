...
let
  cfg = config.programs.void;
in
{
  options.programs.void = {
    enable = lib.mkEnableOption "Void voice dictation app with Wayland auto-paste support";
    ...
    users = lib.mkOption {
      ...
      description = ''
        ...
        ydotool group (to reach the ydotoold socket) and the uinput group (Void's
  ...
  config = lib.mkIf cfg.enable {
    ...
    # do this, and Void's linux-fast-paste --uinput needs /dev/uinput directly.
