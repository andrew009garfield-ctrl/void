...
```bash
# macOS
/Applications/Void.app/Contents/MacOS/Void --log-level=debug

# Windows
Void.exe --log-level=debug
```
...
```
VOID_LOG_LEVEL=debug
```

**Env file locations:**

- macOS: `~/Library/Application Support/Void/.env`
- Windows: `%APPDATA%\Void\.env`
- Linux: `~/.config/Void/.env`

## Log File Locations

- **macOS**: `~/Library/Application Support/Void/logs/debug-*.log`
- **Windows**: `%APPDATA%\Void\logs\debug-*.log`
- **Linux**: `~/.config/Void/logs/debug-*.log`
...
- Remove `VOID_LOG_LEVEL` from `.env`
