...
2. Ensure Void is listed and enabled
...
3. Ensure Void is listed and enabled
...
4. Clear model cache: `rm -rf ~/.cache/void/whisper-models`
5. Try cloud transcription as fallback

**GPU acceleration (CUDA / Vulkan):** If the GPU-accelerated whisper-server crashes at startup (unsupported GPU, out of VRAM), Void automatically restarts it on CPU, retries the same request, and shows a "using CPU instead" notice — the dictation still completes. GPU acceleration can be toggled off from the GPU card in the transcription model picker.
...
2. Void chooses a compositor-specific paste method:
...
3. Restart Void after installing

Void tries clipboard methods in order: `wl-copy` (most reliable) → renderer `navigator.clipboard` → X11 fallback.

On GNOME and KDE, the first automatic paste can show a remote-interaction permission dialog. Approval is remembered in `~/.cache/void/portal-paste-token`; revoking permission or invalidating that token makes the prompt return. If automatic paste fails, the transcription remains in the clipboard for manual paste.
...
4. Restart Void and start meeting transcription again
5. No screen-share chooser is expected for Linux system audio; Void captures the default sink monitor directly through PipeWire
...
1. Grant Screen Recording permission: System Settings → Privacy & Security → Screen Recording → enable Void
...
4. Remember there is no separate assistant window. Press the Voice Assistant hotkey (or use the pill) and speak: highlighted text is edited in place; with auto-paste enabled, an answer pastes at a focused writable cursor. Without a writable cursor—or if the target changes while the answer is generated—Void leaves the field untouched, opens the floating panel attached to the dictation pill, and copies the completed answer to the clipboard for manual paste
...
1. Check the system tray (click the `^` caret) for the Void icon
2. Run with debug logging: `Void.exe --log-level=debug`
3. Try disabling GPU acceleration: `Void.exe --disable-gpu`

**Antivirus / Windows Defender blocking binaries:**

whisper.cpp and FFmpeg may be quarantined silently. Add Void to exclusions: Settings → Virus & threat protection → Exclusions.

**Permission errors:**

Right-click Void → Run as administrator (or set permanently in Properties → Compatibility).

**Firewall blocking cloud mode:**

Allow Void through Windows Firewall when using cloud transcription providers.

**Firewall prompt for sherpa-onnx (local Parakeet transcription):**

Windows may ask whether to allow `sherpa-onnx-ws-win32-x64` on public and private networks the first time local Parakeet transcription starts. The bundled sherpa-onnx server only serves Void itself over `127.0.0.1`, but it has no loopback-only bind option, so Windows sees it listening on all interfaces. Either choice is safe — Windows never filters loopback traffic, so transcription works even if you click Cancel. All-users installs register a firewall rule that blocks outside access and suppresses the prompt entirely; per-user and portable builds may still see it once.

**Complete reset (after uninstalling):**

```batch
rd /s /q "%APPDATA%\Void"
rd /s /q "%LOCALAPPDATA%\Void"
```

Then reinstall.

**Logs location:** `%APPDATA%\Void\logs\`
...
3. Open an issue at https://github.com/Void/void/issues with:
   - OS version
   - Void version
